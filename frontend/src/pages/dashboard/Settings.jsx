  ...
  if (error) {
     return (
       <div className="flex flex-col items-center justify-center py-32 text-center font-poppins">
         ...
         <button onClick={fetchData} className="mt-8 px-10 py-3 bg-slate-800 hover:bg-slate-700 border border-white/10 text-white rounded-xl text-[11px] font-bold uppercase tracking-widest active:scale-95 transition-all">Retry Decryption</button>
       </div>
     );
  }
    ...
    >
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div className="lg:col-span-3 space-y-6">
          <AnimatePresence mode="wait">
            {activeTab === 'profile' && (
              <motion.div 
                ...
                className="bg-slate-800/20 border border-white/5 rounded-3xl p-8 relative overflow-hidden group"
              >
                <h3 className="text-lg font-bold text-white mb-8 tracking-tight uppercase">Profile Parameters</h3>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="space-y-2">
                    ...
                    <input 
                      ...
                      className="w-full bg-slate-900 border border-white/10 rounded-2xl p-4 text-sm font-semibold text-white focus:outline-none focus:border-blue-500/50 transition-all"
                    />
                  </div>
                  <div className="space-y-2">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      ...
                      <input 
                        ...
                        value={formData.city}
                        onChange={handleInputChange}
                        className="w-full bg-slate-900 border border-white/10 rounded-2xl p-4 pl-12 text-sm font-semibold text-white focus:outline-none focus:border-blue-500/50 transition-all"
                      />
                    </div>
                  </div>
                  <div className="space-y-2">
                ...
                <div className="mt-12 flex items-center justify-between border-t border-white/5 pt-8">
                  ...
                  <button 
                    ...
                    className={`flex items-center gap-2 px-8 py-4 rounded-2xl text-xs font-bold uppercase tracking-widest transition shadow-2xl active:scale-95 disabled:opacity-50 ${
                      success ? 'bg-emerald-600 shadow-emerald-500/20' : 'bg-blue-600 shadow-blue-500/20 hover:bg-blue-500'
                    }`}
                  >
            ...
            {activeTab === 'security' && (
              <motion.div 
                ...
                animate={{ opacity: 1, scale: 1 }}
                exit={{ opacity: 0, scale: 0.98 }}
                className="bg-slate-800/20 border border-white/5 rounded-3xl p-8 space-y-8"
              >
                <div className="flex items-center justify-between">
                   <h3 className="text-lg font-bold text-white tracking-tight uppercase">Encryption Keys</h3>
                   <span className="text-[9px] font-bold text-emerald-500 bg-emerald-500/5 px-3 py-1 rounded-full border border-emerald-500/10 uppercase tracking-widest">Active AES-256</span>
                </div>
                
                <div className="space-y-6">
                   <div className="p-6 bg-slate-950/40 rounded-2xl border border-white/5 group hover:border-blue-500/30 transition-all">
                      <div className="flex items-center justify-between mb-4">
                         <div className="flex items-center gap-3">
                            <div className="p-2 bg-blue-500/10 rounded-lg text-blue-500"><Key size={18}/></div>
                            <span className="text-sm font-bold text-white uppercase tracking-wider">Aegis Public Key</span>
                         </div>
                         <button className="text-[10px] font-bold text-blue-400 hover:text-white uppercase tracking-widest transition-colors" onClick={() => alert("Rotating encryption vectors...")}>Rotate Key</button>
                      </div>
                      <code className="text-[11px] text-slate-500 break-all font-mono block p-3 bg-slate-900/50 rounded-xl border border-white/5">
                   ...
                   <div className="p-6 bg-slate-950/40 rounded-2xl border border-white/5">
                      <div className="flex items-center justify-between mb-4">
                         <div className="flex items-center gap-3">
                            ...
                            <span className="text-sm font-bold text-white uppercase tracking-wider">Neural Signature</span>
                         </div>
                         <span className="text-[10px] font-bold text-slate-600 uppercase tracking-widest italic">Verified via Swiggy/Zomato</span>
                      </div>
            ...
            {activeTab === 'nodes' && (
              <motion.div 
                ...
                className="bg-slate-800/20 border border-white/5 rounded-3xl p-8 space-y-8"
              >
                <h3 className="text-lg font-bold text-white tracking-tight uppercase">Active Guard Nodes</h3>
                   ...
                   {[
                     ...
                     <div key={i} className="p-5 bg-slate-950/40 rounded-2xl border border-white/5 flex items-center justify-between group hover:bg-slate-900/40 transition-all">
                        <div className="flex items-center gap-4">
                           <div className="p-2.5 bg-slate-800 rounded-xl">{node.icon}</div>
                           <div>
                              <p className="text-sm font-bold text-white uppercase tracking-wider leading-none mb-1">{node.name}</p>
                              <p className="text-[10px] text-slate-600 font-mono tracking-tight">{node.ip}</p>
                           </div>
                        </div>
                        <div className="flex items-center gap-2">
                           <div className={`w-1.5 h-1.5 rounded-full ${node.status === 'Online' ? 'bg-emerald-500 animate-pulse' : 'bg-slate-600'}`}></div>
                ...
                <div className="p-6 bg-blue-500/5 rounded-2xl border border-blue-500/10 flex items-center gap-4">
                   ...
                   <p className="text-[10px] text-blue-300 font-medium uppercase tracking-widest leading-relaxed">Network topology optimized for parametric insurance. Latency between your localized node and Aegis-HQ is currently 14ms.</p>
                </div>
              </motion.div>
            )}
            ...
            {activeTab === 'notifications' && (
              ...
              >
                <div className="flex items-center justify-between mb-4">
                   <h3 className="text-lg font-bold text-white tracking-tight uppercase">Alert Protocols</h3>
                   <button 
                    onClick={handleSave}
                ...
                <div className="space-y-4">
                   {[
                     ...
                     <div key={i} className="flex items-center justify-between p-6 bg-slate-950/20 rounded-2xl border border-white/5 hover:bg-slate-950/40 transition-all">
                        ...
                        <div 
                          ...
                          className={`w-12 h-6 rounded-full p-1 cursor-pointer transition-all ${formData[item.id] ? 'bg-blue-600' : 'bg-slate-800'}`}
                        >
          ...
          <div className="bg-slate-800/10 border border-white/5 rounded-3xl p-8 hover:bg-slate-800/20 transition-all duration-300 group cursor-pointer">
             <div className="flex items-center justify-between">
                <div className="flex gap-4 items-center">
                  <div className="p-3 bg-indigo-500/5 rounded-xl text-indigo-400 group-hover:bg-indigo-500/10 transition-colors border border-indigo-500/10"><Shield size={20}/></div>
                  <div>
                    <h4 className="text-sm font-bold text-white uppercase tracking-wider mb-1">Aegis Guard Protocol</h4>
                    <p className="text-[10px] text-slate-500 font-medium uppercase tracking-widest">Enhanced biometric encryption status</p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                   <span className="text-[10px] font-bold text-emerald-500 uppercase tracking-[0.2em]">Validated</span>
                   <ChevronRight size={18} className="text-slate-700 group-hover:translate-x-1 group-hover:text-white transition-all" />
                </div>
             </div>
          </div>
        </div>
